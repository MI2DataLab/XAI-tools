...
This report aims to present the capabilities of the package `glmnet` in the field of generalized linear galss-box models.
...
model <- glmnet(X, as.factor(titanic_imputed$survived), family = "binomial")
```
...
```{r}
coef(model, model$lambda[length(model$lambda)])
```
